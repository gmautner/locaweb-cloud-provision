...
- **Firewall rules**: CloudStack firewall rules control inbound access. The web VM allows TCP ports 22 (SSH), 80 (HTTP), and 443 (HTTPS). Worker VMs allow only TCP port 22 (SSH). Accessory VMs always allow TCP port 22 (SSH) and may expose additional ports via the optional `ports` field in the accessories JSON (e.g., `"ports": "5432"`). All rules use CIDR `0.0.0.0/0` (unrestricted source).
